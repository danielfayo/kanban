...
const StatusSelect: React.FC<StatusSelectProps> = ({ placeholder,board, changeStatus }) => {
  ...
  return (
    <Select onValueChange={changeStatus}>
      ...
      <SelectContent className="w-full bg-White dark:bg-Very-Dark-Grey py-2 flex flex-col gap-2">
        {boards?.map(each => (
          <SelectItem className=" cursor-pointer w-full" key={each.id} value={each.name}>{each.name}</SelectItem>
        ))}
      </SelectContent>
    </Select>
